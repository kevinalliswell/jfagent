...
- `GET /api/health` (open)
- `GET /api/auth/mode` (open)
- `POST /api/auth/register` (open unless `REGISTRATION_MODE=closed`)
...
- `GET /api/assets/{asset_id}/download` (capability URL, no auth header needed)
- `GET /api/admin/knowledge/status` (admin role)
- `POST /api/admin/knowledge/upload` (admin role)
- `GET /api/admin/runtime/status` (admin role)
- `GET /api/admin/users` (admin role)
- `POST /api/admin/users/grant` (admin role)
- `GET /api/admin/licenses` (admin role)
- `POST /api/admin/licenses` (admin role)

Auth model: Bearer JWT (HMAC-SHA256; secret from `JWT_SECRET` or generated and
persisted in SQLite). Passwords hashed with scrypt. Roles: `admin`/`user`.
`AUTH_DISABLED=1` turns every request into a synthetic local admin
(single-operator mode). Sessions and projects carry `user_id`; non-admins only
see their own records, and the bootstrap admin claims legacy ownerless rows.

Billing model: formal export consumes 1 export credit
(402 `PAYMENT_REQUIRED` for confirmation, 402 `NO_CREDITS` when exhausted);
free preview is unlimited; admins bypass the debit. License codes carry
credits, are generated by admins, redeemed once, and all movements are written
to `credit_transactions`.

Current behavior:

- Deterministic expert engine in `server/rulesEngine.ts` (implements `docs/specs-future/rules.md`): UPS/cooling/battery sizing, internal reference BOM estimate, P0/P1 risk rules, audit log, confirmed/provisional/blocked status. The engine drives `triggered_risks`, the extended `SuggestionSummary`, fallback chat copy, LLM prompt context, and export payload numbers.
- Per-session chat history (`messages`, capped at 200) persisted in the session snapshot, returned by `GET /api/session`, and replayed (last 12 turns) to the LLM.
- SQLite-backed user, license, credit-transaction, session, project, export-asset, and knowledge-upload persistence under `data/jfagent.sqlite` (compose mounts a `jfagent_data` volume).
...
- `server/rulesEngine.ts`: deterministic expert engine — UPS/cooling/battery calculation, internal BOM cost estimate, risk rules, audit log, suggestion builder.
- `server/auth.ts`: accounts (scrypt password hashing), JWT sign/verify, bootstrap admin, registration modes, export-credit debits/grants, license code generation and redemption.
- `server/sessionService.ts`: session behavior, message history, deterministic extraction, engine wiring, user scoping, credit-gated export, API response assembly.
- `server/projectService.ts`: project domain store with per-user ownership, summary cloning, and SQLite-backed project persistence wiring.
- `server/persistence.ts`: SQLite persistence helpers for users, licenses, credit transactions, app secrets, sessions, projects, export assets, knowledge uploads, upload jobs, and retrieval audit rows.
...
Location: `api_spec.md` at the repo root plus `docs/specs-future/*.md`. Most are
FUTURE/aspirational design — read each file's STATUS banner and
`docs/specs-future/README.md` before relying on them.
...
- `docs/specs-future/rules.md`: expert rules and calculation engines (FUTURE).
- `docs/specs-future/knowledge_base.md`: RAG ingestion, tagging, verification, retrieval policy (FUTURE).
- `docs/specs-future/templates.md`: proposal/Word rendering rules (PARTIAL).
...
| ------- | ----------------------- | ------------------------------------ |
| State authority | Stateless per call; the frontend passes the full `session` in and back. No persistence. | Server-authoritative; `loadOrCreateSession` + SQLite, `state_version` owned by the server, retrieval audit logged. |
| Auth & billing | None — demo runs without login; export uses the simulated 99 RMB willingness modal. | Real accounts (JWT), per-user data isolation, export credits + license codes, 402 `NO_CREDITS` gate. |
| LLM | None. `agent_runtime` is always `fallback`/`provider: "mock"`. | Optional OpenAI-compatible call with conversation history + engine context; `agent_inference` candidates filtered by `llmProtectedSources`. |
| Conversation text | Rich canned `ai_response`/`quick_replies` per branch. | Engine-backed `fallbackAgentResponse` (sizing + estimate + prompts) unless the real LLM answers. |
| Conversation persistence | Browser memory only; lost on reload. | `messages` persisted per session and restored via `GET /api/session`. |
| Suggestion math | Simple `buildSuggestion(rackCount)` (3 kW/柜, ≤12 racks → 40 kW AC). No estimate fields. | Full `rulesEngine` evaluation: avg power/redundancy honored, catalog cooling units, battery count, BOM estimate, calculation status. |
| Budget risk | Fixed threshold: budget < 450,000 triggers. | BOM×1.3 comparison when estimable (rich text with figures); 450k heuristic only as fallback. |
| Export artifact | Canned asset, `download_url: "#mock-download"`, fixed `included_chapters` list. No real payload. | Real `ExportPayloadV1` + `python-docx` render + LibreOffice PDF preview; `included_chapters` derived from `chapter_plan`; credits recorded. |
| Projects / snapshot | 3 fixed seed projects; snapshot only for the one seed session. | Real project domain + persistence + per-user ownership (`projectService`). |

### Current parity status

As of the 2026-06-12 commercial v1, mock mode is intentionally a **demo
subset** of the backend product. The rows above list the accepted divergences;
the still-shared semantics are:

- override safety on listed editable fields
- field-derived FSM / `export_status` derivation (`src/mockSessionDerivation.ts`)
- risk IDs and `structuralNote`/`pduNote` wording for the shared rules
- the simple-threshold part of budget-risk triggering (mock always; backend
  only when the BOM estimate is blocked)

Regression coverage lives in `tests/presalesCockpit.test.ts` (mock-side pure
derivation checks), `tests/backendDeterministicLogic.test.ts` (backend
deterministic behavior), and `tests/rulesEngine.test.ts` (engine spec matrix).
Do not extend mock mode to mirror auth, credits, message persistence, or the
estimate engine — record any new gap in the table instead.
